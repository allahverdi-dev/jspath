...
import {
  ...
  paddleCatalog, paddleEnvironment, sha256Hex, verifyPaddleSignature,
...
Deno.serve(async (request) => {
  ...
  const finish = async (status: string, error?: string) => {
    await admin.from('billing_events')
      ...
      .eq('provider', 'paddle').eq('event_key', eventId);
  };
  ...
  try {
    ...
    const record = normalizePaddleSubscription({ subscription, catalog });
    ...
    const { error: upsertError } = await admin
      .from('subscriptions')
      .upsert({ ...record, user_id: resolvedUserId }, { onConflict: 'provider,provider_subscription_id' });
