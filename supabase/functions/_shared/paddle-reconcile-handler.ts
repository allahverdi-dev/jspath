...
import {
  getPaddleSubscription, getPaddleTransaction, isFresherThanStored, canUsePaddleEnvironment,
  jspathUserIdFrom, normalizePaddleSubscription, paddleCatalogFor,
} from './paddle.js';
...
export function createPaddleReconcileHandler(environment: 'sandbox' | 'production') {
  ...
  const applySubscription = async (subscription: Record<string, unknown>) => {
    const record = normalizePaddleSubscription({ subscription, catalog, environment, now });
  ...
  try {
    ...
    for (const row of existing ?? []) {
      ...
      const subscription = await getPaddleSubscription(row.provider_subscription_id, environment);
    ...
    for (const session of pending ?? []) {
      const transaction = await getPaddleTransaction(session.provider_transaction_id, environment);
      const subscriptionId = typeof transaction?.subscription_id === 'string' ? transaction.subscription_id : null;
      if (!subscriptionId) continue;

      const subscription = await getPaddleSubscription(subscriptionId, environment);
